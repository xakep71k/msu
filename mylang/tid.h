...
public:
    ...
    const std::string &get_current_func() const
    {
        return current_func_;
    }

    void set_current_func(const std::string &func)
    {
        current_func_ = func;
    }
    ...
    {
        return space_[current_func_][i];
    }

    void push_back(const Ident &ident)
    {
        space_[current_func_].push_back(ident);
    }

    operator std::vector<Ident> &()
    {
        return space_[current_func_];
    }
};
